...
let handler = async (m, { conn, usedPrefix, DevMode }) => { 
    try { 
        ...
        if (global.db.data.users[m.sender].healt > 79) {
            ...
            let exp = (Math.floor(Math.random() * 350) + (kuda * 70))
            let uang = `${Math.floor(Math.random() * 350)}`.trim() 
            ...
            let sampah = `${Math.floor(Math.random() * 350)}`.trim()
...
// handler.help = ['adventure', 'berpetualang', 'mulung', 'work']
// handler.tags = ['rpg']
